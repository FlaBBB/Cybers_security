...
class globfuscator:
    ...
    def question_conv(self, in_string: str, seed: int) -> str:
        if seed < 1 or seed > self.get_max_question_conv(in_string, True):
            return in_string

        max_conversable = self.get_max_question_conv(in_string)

        pattern = bin(seed)[2:].zfill(max_conversable)
    ...
    def __globfuscating(self, name_path: str, dirs:list, is_env:bool = False) -> str:
        ...
        for a in range(max_asterisk_conv, -1, -1):
            ...
            max_question_conv = self.get_max_question_conv(asterisk_path, True)

            for b in range(max_question_conv, -1, -1):
                globfuscated_path = self.question_conv(asterisk_path, b)
