password = """.wrapper:has(.byte:nth-child(1) .latch:nth-child(7) .latch__reset:active) .checker:nth-of-type(2) .checker__state:nth-child(1) {
...
splitting = password.split(".wrapper")

filtered = []
# filtering
for s in splitting:
    if "translateX(-100%)" in s:
        filtered.append(s)
...
for f in filtered:
    # getting num bytes
    f_num = f.split(".byte:nth-child(")
    num = int(f_num[1].split(")")[0])

    # getting num latch
    f_num_latch = f.split(".latch:nth-child(")
    num_latch = int(f_num_latch[1].split(")")[0])

    # getting num state
    f_state = f.split(".latch__")
    state = f_state[1].split(":active)")[0]
    if the_pass.get(num) == None:
        the_pass[num] = {}
    assert the_pass[num].get(num_latch) == None
    if state == "set":
        the_pass[num][num_latch] = 1
    else:
        the_pass[num][num_latch] = 0
...
pass_key = ""
i = 1
while the_pass.get(i, False):
    binary = 0
    for j in range(1, 9):
        binary |= the_pass[i].get(j, 0) << (8 - j)
    pass_key += chr(binary)
    i += 1
