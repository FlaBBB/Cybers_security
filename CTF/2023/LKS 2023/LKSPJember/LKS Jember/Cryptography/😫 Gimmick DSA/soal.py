...
def validasi(data):
    ...
    for i in tes:
        if data.count(i) != 1:
            print(
                "Tidak boleh mengandung kata dan/atau menambahkan atribut komponen secara langsung!"
            )
            exit()
...
def check(data):
    ...
    if verifikasi_digital(kunci_rahasia, parsed_data) != signed_hash:
        print(
            "Integritas telah rusak. Tolong input kembali username dan password Anda."
        )
        return
...
def main():
    ...
    wrapper = (
        b"pw="
        + hashlib.sha512(password).hexdigest().encode()
        + b"|role="
        + default_role
        + b"|nama="
        + username
